...
async function comprobarLlamada(datos) {
  ...
  if (llamadaIndividual) {
    ...
    db.ref("groups/" + groupId + "/participants").on(
      "value",
      async (snapshot) => {
...
async function makeCallGrupal() {
  db.ref("calls/" + currentUserId).on("child_added", async (snapshot) => {
    console.log(
      "SNAPSHOT UNDEFINED: -------------------" + JSON.stringify(snapshot.val())
    );
...
async function answerCallGrupal() {
  ...
  db.ref("calls/" + currentUserId).on("value", async (snapshot) => {
    ...
    for (let key in ofertas) {
      ...
      await peerConnection[key].setRemoteDescription(
        new RTCSessionDescription(ofertas[key])
      );
      ...
      await peerConnection[key].setLocalDescription(
  ...
  if (llamadaIndividual) {
    ...
    db.ref("groups/" + groupId + "/participants").on(
      ...
      async (snapshot) => {
        ...
        for (let userId in aycaramba) {
          if (aycaramba[userId] != currentUserId) {
            db.ref("/calls/" + aycaramba[userId] + "/candidates")
              .push()
              .set({ iceCandidate: payload });
          }
        }
      }
    );
...
export default class VideoTest extends React.Component {
  ...
  componentDidMount = () => {
    ...
    for (let key in peerConnection) {
      peerConnection[key].onaddstream = (e) => {
        ...
        for (
          var i = this.state.remoteStream.length >= 1 ? 1 : 0;
          i < this.state.remoteStream.length;
          i++
        ) {
          let objectAux = (
            <RTCView
              ...
              streamURL={
                this.state.remoteStream[i] && this.state.remoteStream[i].toURL()
              }
            />
          );
  ...
  setRemoteDescription = () => {
    ...
    for (var ayay in peerConnection) {
      peerConnection[ayay].setRemoteDescription(
  ...
  addCandidate = () => {
    ...
    console.log(
      "DIKOSAJHDIOPUASHDUIOSAHDUIASHGDUIOYSAHDGIOUYASHDUIOSAHDIUOASHDUIOSAHDUIOSAHDIUOASHDUIYASOHDIUOSAHDUIODAHUI"
    );
  ...
  render() {
    const { localStream, remoteStream, streams } = this.state;
